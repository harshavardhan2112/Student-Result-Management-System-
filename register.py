...
class Register:
    def __init__(self,root):
        ...
        self.left=ImageTk.PhotoImage(file="bg1.jpg")
        ...
        contact=Label(frame1,text="Contact No.",font=("times new roman",15,"bold"),bg="white",fg="gray").place(x=370,y=170)
        self.txt_contact=Entry(frame1,font=("times new roman",15),bg="lightgray")
        self.txt_contact.place(x=370,y=200,width=250)

        email=Label(frame1,text="Email",font=("times new roman",15,"bold"),bg="white",fg="gray").place(x=50,y=170)
        ...
        self.var_chk=IntVar()
        chk=Checkbutton(frame1,text="I Agree The Terms & Conditions",variable=self.var_chk,onvalue=1,offvalue=0,bg="white",font=("times new roman",12)).place(x=50,y=380,width=250)


        btn_register=Button(frame1,text="Register Now",bd=0,cursor="hand2",bg="green",command=self.register_data).place(x=50,y=420,width=250,height=30)
        btn_login=Button(self.root,text="Sign In",command=self.login_window,font=("times new roman",20)).place(x=850,y=510,width=250,height=30)
    ...
    def register_data(self):
        if (self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.txt_answer.get()=="" or self.cmb_quest['values']=="Select" or self.txt_password.get()=="" ):
            messagebox.showerror("Error","All Field are Required",parent=self.root)
        elif(len(self.txt_contact.get())!=10):
            messagebox.showerror("Error","Contact Number should be of 10 digits")
            ...
            try:
                ...
                if row!=None:
                    ...
                    cur.execute("insert into emp3(f_name,l_name,contact,email,question,answer,password) values(%s,%s,%s,%s,%s,%s,%s)",(self.txt_fname.get(),self.txt_lname.get(),self.txt_contact.get(),self.txt_email.get(),self.cmb_quest.get(),self.txt_answer.get(),self.txt_password.get(),))
                    con.commit()
                    con.close()
